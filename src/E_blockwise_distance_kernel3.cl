    ...
    const int n_filters)
{
	...
	__local float8 l_FilterBank[108];
  ...
  int out_y = out_pix_y0 + get_local_id(0);
    for (int ii=0; ii<BLOCK_MULT; ii++){
        ...
        for (int jj=0; jj<BLOCK_MULT; jj++){
            ...
            if ((out_y < out_pix_y_stop) && (out_x < out_pix_x_stop)){ 
				...
				curval += (tempval.s0+tempval.s1+tempval.s2+tempval.s3+tempval.s4+tempval.s5+tempval.s6+tempval.s7);
			//	curval += ((int)tempval.s0&(int)tempval.s1&(int)tempval.s2&(int)tempval.s3&(int)tempval.s4&(int)tempval.s5&(int)tempval.s6&(int)tempval.s7);
	...
	/*			if ((tempval.s0) > curval){
				...
				tempval = fabs(tempval);
				curval += (tempval.s0+tempval.s1+tempval.s2+tempval.s3);
		//		curval += ((int)tempval.s0&(int)tempval.s1&(int)tempval.s2&(int)tempval.s3);
				/*
					if ((tempval.s0) > curval){
                        curid = (n_filters/8)*8;
                        curval = (tempval.s0);
