        ...
        __global float* out, const int o_pitch_f)
{
	...
	float8 mysub = b[b_mul + get_local_id(1)];
    float8 dst = 0;
    ...
    for (int i=0; i < a_width/8; i+=BLOCK_SIZE)
    {
		...
		float8 diff0 = a[a_ind + 0]  - mysub;
		dst=mad(diff0, diff0, dst);
        //dst += diff0 * diff0;

		float8 diff1 = a[a_ind + 1]  - mysub;
		dst= mad(diff1, diff1, dst);
        //dst += diff1 * diff1;

		float8 diff2 = a[a_ind + 2]  - mysub;
		dst=mad(diff2, diff2, dst);
        //dst += diff2 * diff2;

		float8 diff3 = a[a_ind + 3]  - mysub;
		dst=mad(diff3, diff3, dst);
        //dst += diff3 * diff3;

		float8 diff4 = a[a_ind + 4]  - mysub;
		dst=mad(diff4, diff4, dst);
        //dst += diff4 * diff4;

		float8 diff5 = a[a_ind + 5]  - mysub;
		dst=mad(diff5, diff5, dst);
        //dst += diff5 * diff5;

		float8 diff6 = a[a_ind + 6]  - mysub;
		dst=mad(diff6, diff6, dst);
        //dst += diff6 * diff6;

		float8 diff7 = a[a_ind + 7]  - mysub;
		dst=mad(diff7, diff7, dst);
        //dst += diff7 * diff7;

		float8 diff8 = a[a_ind + 8]  - mysub;
		dst=mad(diff8, diff8, dst);
        //dst += diff8 * diff8;

		float8 diff9 = a[a_ind + 9]  - mysub;
		dst=mad(diff9, diff9, dst);
        //dst += diff9 * diff9;

		float8 diff10 = a[a_ind + 10]  - mysub;
		dst=mad(diff10, diff10, dst);
        //dst += diff10 * diff10;

		float8 diff11 = a[a_ind + 11]  - mysub;
		dst=mad(diff11, diff11, dst);
        //dst += diff11 * diff11;

		float8 diff12 = a[a_ind + 12]  - mysub;
		dst=mad(diff12, diff12, dst);
        //dst += diff12 * diff12;

		float8 diff13 = a[a_ind + 13]  - mysub;
		dst=mad(diff13, diff13, dst);
        //dst += diff13 * diff13;

		float8 diff14 = a[a_ind + 14]  - mysub;
		dst=mad(diff14, diff14, dst);
        //dst += diff14 * diff14;
		
		float8 diff15 = a[a_ind + 15]  - mysub;
