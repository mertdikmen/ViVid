...
program_list[0] = clCreateProgramWithSource(GPUContext, 1, (const char **) &program_source, NULL, &err);
        ...
        //
        err = clBuildProgram(program_list[0], 0, NULL, NULL, NULL, NULL);
        if (err != CL_SUCCESS) {
            size_t len;
            char buffer[2048];
	...
	err =  clSetKernelArg(kernel_list[0], 0, sizeof (DeviceMatrixCL), &features_train);
    ...
    if (err != CL_SUCCESS) {
		...
		const size_t global_work_size[2]={features_train->width,features_train->height}; 
