						...
						int DATA_SIZE)
{
    ...
    if ((*kern_y) == KERN_SIZE) {
        ...
        output[ offset_t*pitch_t+ (y-KERN_SIZE+1)*pitch_y+
		offset_x + get_local_id(1)]
		= (*sum);
        (*sum) = 0;
        (*kern_y) = 0;
...
__kernel void  do_convolution3_11(
	...
	)
{
    ...
    __local float kern[11][11][11];
  for (unsigned int t=0; t < KERN_SIZE; t++)
    {
        if (get_local_id(1) < KERN_SIZE)
        {
            kern[t][get_local_id(0)][get_local_id(1)] =
			_kernel[ 
			  t*kernel_pitch_tf+ 
			  get_local_id(0)*kernel_pitch_yf+
			  get_local_id(1)];
        }
    }
	...
	// Data_size = 11+16-1 = 20
    __local float data[2][1][26];
  ...
  /*  do_convolution3_load
	(data[0],
	 video, offset_x, offset_t,0,output_pitch_tf,output_pitch_yf,
	 KERN_SIZE, DATA_SIZE, SWATH_SIZE,
	 );
		...
		*/
{
    ...
    if (kern_y == KERN_SIZE) {
        // Write out the solution
        output[ offset_t*output_pitch_tf+ (y-KERN_SIZE+1)*output_pitch_yf+
		offset_x + get_local_id(1)]
		= sum;
		sum = 0;
        kern_y = 0;
