...
__kernel void  do_convolution5_11(
	...
	)
{
        ...
        for (unsigned int y=0; y < DATA_SIZE; y += KERN_SIZE)
        {
            ...
            if (my_y < DATA_SIZE)
            {
                for (unsigned int x=0; x < DATA_SIZE; x += 16)
                {
                    ...
                    if (my_x < DATA_SIZE) {
                        data[my_x][my_y][t] =
						video[(offset_t + my_x)*video_pitch_tf
   ...
   barrier(CLK_GLOBAL_MEM_FENCE | CLK_LOCAL_MEM_FENCE);	
    ...
    for (unsigned int t=0; t < KERN_SIZE; t++) {
        for (unsigned int y=0; y < KERN_SIZE; y++) {
            for (unsigned int x=0; x < KERN_SIZE; x++) {
				...
				constant_kernel[((((KERN_SIZE-t-1) * 
				KERN_SIZE) + ( KERN_SIZE-y-1))
				* KERN_SIZE) + (KERN_SIZE-x-1)];
    ...
    // Write out the solution
		 ...
		 output[(offset_t + base_t)*video_pitch_tf
