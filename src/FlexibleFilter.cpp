...
{
	if ((dst->dim_t != 2) || 
		(dst->dim_y != frame->height) || 
		(dst->dim_x != frame->width / nchannels) || 
		(dst->my_context != frame->my_context))
	{
		dst = makeDeviceMatrixCL3D(2, frame->height, frame->width / nchannels, frame->my_context->getDeviceType());
	}

	dist_filter2_d3_cl(frame.get(), dim_t, nchannels, dst.get(), optype);
}
