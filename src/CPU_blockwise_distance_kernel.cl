  ...
  int out_y = out_pix_y0 + get_local_id(0);
    for (int ii=0; ii<BLOCK_MULT; ii++){
        ...
        for (int jj=0; jj<BLOCK_MULT; jj++){
            ...
            int fi = 0;
		/*	float8 img0 = {image_cache[cyi*cache_size+cxi], image_cache[cyi*cache_size+cxi],
							image_cache[cyi*cache_size+cxi], image_cache[cyi*cache_size+cxi],
							image_cache[cyi*cache_size+cxi], image_cache[cyi*cache_size+cxi],
							image_cache[cyi*cache_size+cxi], image_cache[cyi*cache_size+cxi]};

			float8 img1 = {image_cache[cyi*cache_size+cxi+1], image_cache[cyi*cache_size+cxi+1],
							image_cache[cyi*cache_size+cxi+1], image_cache[cyi*cache_size+cxi+1],
							image_cache[cyi*cache_size+cxi+1], image_cache[cyi*cache_size+cxi+1],
							image_cache[cyi*cache_size+cxi+1], image_cache[cyi*cache_size+cxi+1]};

			float8 img2 = {image_cache[cyi*cache_size+cxi+2], image_cache[cyi*cache_size+cxi+2],
							image_cache[cyi*cache_size+cxi+2], image_cache[cyi*cache_size+cxi+2],
							image_cache[cyi*cache_size+cxi+2], image_cache[cyi*cache_size+cxi+2],
							image_cache[cyi*cache_size+cxi+2], image_cache[cyi*cache_size+cxi+2]};

			float8 img3 = {image_cache[(cyi+1)*cache_size+cxi], image_cache[(cyi+1)*cache_size+cxi],
							image_cache[(cyi+1)*cache_size+cxi], image_cache[(cyi+1)*cache_size+cxi],
							image_cache[(cyi+1)*cache_size+cxi], image_cache[(cyi+1)*cache_size+cxi],
							image_cache[(cyi+1)*cache_size+cxi], image_cache[(cyi+1)*cache_size+cxi]};

			float8 img4 = {image_cache[(cyi+1)*cache_size+cxi+1], image_cache[(cyi+1)*cache_size+cxi+1],
							image_cache[(cyi+1)*cache_size+cxi+1], image_cache[(cyi+1)*cache_size+cxi+1],
							image_cache[(cyi+1)*cache_size+cxi+1], image_cache[(cyi+1)*cache_size+cxi+1],
							image_cache[(cyi+1)*cache_size+cxi+1], image_cache[(cyi+1)*cache_size+cxi+1]};

			float8 img5 = {image_cache[(cyi+1)*cache_size+cxi+2], image_cache[(cyi+1)*cache_size+cxi+2],
							image_cache[(cyi+1)*cache_size+cxi+2], image_cache[(cyi+1)*cache_size+cxi+2],
							image_cache[(cyi+1)*cache_size+cxi+2], image_cache[(cyi+1)*cache_size+cxi+2],
							image_cache[(cyi+1)*cache_size+cxi+2], image_cache[(cyi+1)*cache_size+cxi+2]};

			float8 img6 = {image_cache[(cyi+2)*cache_size+cxi], image_cache[(cyi+2)*cache_size+cxi],
							image_cache[(cyi+2)*cache_size+cxi], image_cache[(cyi+2)*cache_size+cxi],
							image_cache[(cyi+2)*cache_size+cxi], image_cache[(cyi+2)*cache_size+cxi],
							image_cache[(cyi+2)*cache_size+cxi], image_cache[(cyi+2)*cache_size+cxi]};

			float8 img7 = {image_cache[(cyi+2)*cache_size+cxi+1], image_cache[(cyi+2)*cache_size+cxi+1],
							image_cache[(cyi+2)*cache_size+cxi+1], image_cache[(cyi+2)*cache_size+cxi+1],
							image_cache[(cyi+2)*cache_size+cxi+1], image_cache[(cyi+2)*cache_size+cxi+1],
							image_cache[(cyi+2)*cache_size+cxi+1], image_cache[(cyi+2)*cache_size+cxi+1]};

			float8 img8 = {image_cache[(cyi+2)*cache_size+cxi+2], image_cache[(cyi+2)*cache_size+cxi+2],
							image_cache[(cyi+2)*cache_size+cxi+2], image_cache[(cyi+2)*cache_size+cxi+2],
							image_cache[(cyi+2)*cache_size+cxi+2], image_cache[(cyi+2)*cache_size+cxi+2],
							image_cache[(cyi+2)*cache_size+cxi+2], image_cache[(cyi+2)*cache_size+cxi+2]};
*/

			float8 img = {image_cache[cyi*cache_size+cxi], image_cache[cyi*cache_size+cxi+1],
			  image_cache[cyi*cache_size+cxi+2], 
			  image_cache[(cyi+1)*cache_size+cxi],
			  image_cache[(cyi+1)*cache_size+cxi+1], 
			  image_cache[(cyi+1)*cache_size+cxi+2],
			  image_cache[(cyi+2)*cache_size+cxi],
			  image_cache[(cyi+2)*cache_size+cxi+1]};

			float img_leftover = image_cache[(cyi+2)*cache_size+cxi+2];

            if ((out_y < out_pix_y_stop) && (out_x < out_pix_x_stop)){ 
                for (int filter_id=0; filter_id<n_filters; filter_id++){
				// float tempval=0.0f;
                    float tempval = img_leftover * c_FilterBank[fi+8];
					float8 filter = *((float8 *)(&c_FilterBank[fi]));
                     float8 res = img * filter;
				...
				/*		tempval += c_FilterBank[fi++] * image_cache[cyi*cache_size+cxi];
						...
						tempval += c_FilterBank[fi++] * image_cache[(cyi+1)*cache_size+cxi];
						tempval += c_FilterBank[fi++] * image_cache[(cyi+1)*cache_size+cxi+1];
						tempval += c_FilterBank[fi++] * image_cache[(cyi+1)*cache_size+cxi+2];
                        
                        
						tempval += c_FilterBank[fi++] * image_cache[(cyi+2)*cache_size+cxi];
						tempval += c_FilterBank[fi++] * image_cache[(cyi+2)*cache_size+cxi+1];
						tempval += c_FilterBank[fi++] * image_cache[(cyi+2)*cache_size+cxi+2];
                  */       
                 // int8 sel = fabs(tempval) > curval;
				//	curval = select(fabs(tempval), curval, sel);
				//	curid = select(fabs(tempval), curid, sel);
				tempval += dot(res.lo,(float4)(1,1,1,1))+ dot(res.hi,(float4)(1,1,1,1));
				fi += 8;
