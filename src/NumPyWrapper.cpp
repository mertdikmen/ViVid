...
NumPyArray::NumPyArray(const boost::python::object& source)
{
  ...
  // borrowed references.
  PyObject* contig
    = PyArray_FromAny(source.ptr(), PyArray_DescrFromType(NPY_FLOAT),
...
NumPyArray::NumPyArray(unsigned int size)
{
  ...
  PyObject* arr = PyArray_ZEROS(1, &dim, NPY_FLOAT,0);
  handle<> temp(arr);
  array = object(temp);
}
...
NumPyMatrix::NumPyMatrix(unsigned int height, unsigned int width) {
  ...
  PyObject* arr = PyArray_SimpleNew(2, dims, NPY_FLOAT);
  handle<> temp(arr);
  array = object(temp);
}
...
NumPyImage::NumPyImage(const boost::python::object& source)
{
  ...
  // borrowed references.
  PyObject* contig
    = PyArray_FromAny(source.ptr(), PyArray_DescrFromType(NPY_UINT8),
...
unsigned int NumPyArray::size() const {
  return PyArray_DIM(reinterpret_cast<PyArrayObject*>(array.ptr()), 0);
}

float* NumPyArray::data() {
  return (float*)PyArray_DATA(reinterpret_cast<PyArrayObject*>(array.ptr()));
}
...
unsigned int NumPyMatrix::width() const {
  return PyArray_DIM(reinterpret_cast<PyArrayObject*>(array.ptr()), 1);
}
...
unsigned int NumPyMatrix3D::dim_x() const {
  return PyArray_DIM(reinterpret_cast<PyArrayObject*>(array.ptr()), 2);
}
...
unsigned int NumPyImage::depth() const {
  return (PyArray_NDIM(reinterpret_cast<PyArrayObject*>(array.ptr())) == 2) ? 1 : PyArray_DIM(reinterpret_cast<PyArrayObject*>(array.ptr()), 2);
}

unsigned char* NumPyImage::data() {
  return (unsigned char*)PyArray_DATA(reinterpret_cast<PyArrayObject*>(array.ptr()));
}
